function [secModel, addedRxns]=humanSec(model, protein, psim, tempRxns)
  ...
  newCompUniq = unique(newComps);
  newCompUniq = setdiff(newCompUniq,model.comps);
  %find and add new compartements to the secModel
  for i = 1:numel(newCompUniq)
    if strcmp(newCompUniq(i), 'sv') == 1
      secModel.comps = vertcat(secModel.comps,'sv');
      secModel.compNames = vertcat(secModel.compNames,'secretory vesicle');
      secModel.compOutside = vertcat(secModel.compOutside,'c');
    elseif strcmp(newCompUniq(i), 'cv') == 1
      secModel.comps = vertcat(secModel.comps,'cv');
      secModel.compNames = vertcat(secModel.compNames,'clathrin vesicle');
      secModel.compOutside = vertcat(secModel.compOutside,'c');
    elseif strcmp(newCompUniq(i), 'rm') == 1
      secModel.comps = vertcat(secModel.comps,'rm');
      secModel.compNames = vertcat(secModel.compNames,'ER membrane');
      secModel.compOutside = vertcat(secModel.compOutside,'c');
    elseif strcmp(newCompUniq(i), 'gm') == 1
      secModel.comps = vertcat(secModel.comps,'gm');
      secModel.compNames = vertcat(secModel.compNames,'Golgi membrane');
      secModel.compOutside = vertcat(secModel.compOutside,'c');
    elseif strcmp(newCompUniq(i), 'pm') == 1
      secModel.comps = vertcat(secModel.comps,'pm');
      secModel.compNames = vertcat(secModel.compNames,'Plasma membrane');
      secModel.compOutside = vertcat(secModel.compOutside,'s');
    end
  end
